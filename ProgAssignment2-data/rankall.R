...
rank_data <- read.csv("outcome-of-care-measures.csv", colClasses="character")

column <- if (outcome == "heart attack") {
rank_data[, 11] <-suppressWarnings(as.numeric(rank_data[, 11]))
"Hospital.30.Day.Death..Mortality..Rates.from.Heart.Attack"
} else if (outcome == "heart failure") {
rank_data[, 17] <- suppressWarnings(as.numeric(rank_data[, 17]))
"Hospital.30.Day.Death..Mortality..Rates.from.Heart.Failure"
} else if (outcome == "pneumonia") {
rank_data[, 23] <- suppressWarnings(as.numeric(rank_data[, 23]))
"Hospital.30.Day.Death..Mortality..Rates.from.Pneumonia"
} else {
stop("invalid outcome")
}

data_by_state <- split(rank_data[, c("Hospital.Name", "State", column)], rank_data$State)
